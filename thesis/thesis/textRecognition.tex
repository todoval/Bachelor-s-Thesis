...
If the user has provided us with an image of a different resolution, \xxx{the next step}\todo{ceho? jeste nemluvis ani o prvnim stepu a najednou je next} is to either upscale (if the dpi of the input image was lower) or downscale (if the dpi of the input image was higher) the image. In both cases, interpolation is used.\todo{for what? interpolation-based algorithms}

Interpolation \xxx{works by}\todo{tohle neni works by ale input+output} using known data to estimate values at unknown points. It specifically approximates the \xxx{resulting pixel's}\todo{apostrofovy possesives neni mozny pouzivat} color and intensity based on the values at surrounding pixels. When downscaling, this technique therefore computes one color value for each set of pixels, which are the color values of the pixels in the new image. When upscaling, it computes color values of the new pixels by approximating the values of the old pixels around them.

This technique\todo{je to technique, algorithm nebo interpolation? Mozna chces tohle prilepit k predchozimu odstavci, ze Interpolation algorithm attempt to... Mimochodem je extremne vhodny odstranit tak 80\% vyskytu `technique' necim konkretnejsim} tries to minimize the aliasing effect of the resulting image, so the scaled picture does not appear visibly pixelated and its edges are not jagged.

Existing interpolation algorithms can be grouped into two categories: adaptive and non-adaptive.

Adaptive methods change the way they treat various pixels depending on what they are interpolating, \xxx{specifically smooth texture vs. sharp edges}. They are primarily designed to minimize the presence of interpolation artifacts in regions where they are most apparent.

Non-adaptive methods, on the other hand, treat all pixels equally. Their complexity depends on the number of adjacent pixels during interpolation\todo{adjacent pixelu je vzdycky 8, mozna chces rict ze podle toho jak velkej pixel neighborhood zpracovavaji}, which is also the criterion \todo{distinction?} \xxx{by} which the existing methods are classified~\cite{interpolation}. We \xxx{present}\todo{present ma naznak obdarovavani, tohle je Several of the methods are detailed in ...} these methods in~\cref{fig:preprocessInterpolation}.
...
\caption{Comparison of results of several non-adaptive interpolation methods. (Image by~\citet{prajapati2012evaluation}.) \xxx{hele, z tohohle obrazku neni nic videt a neni popsany co se tam vlastne stalo, je to upscaling nebo downscaling? Z jakyho rozliseni na jaky? Proc ty obrazky jsou tak zesedly?}}
\label{fig:preprocessInterpolation}
\end{figure}

\xxx{There also exist}\todo{proc nejsou v predchozi kategorizaci?} higher order interpolations, such as Spline, Sinc, Lanczos or Discrete Wavelet Transforms~\cite{interpolation}. These consider more surrounding pixels and calculate the resulting value through more complex functions, \xxx{like}\todo{`like' znamena neco uplne jinyho; tadyto je `such as'} transforms or sampling functions. The results are more accurate than those of simple calculations at the cost of significantly more time\todo{what kind of time? do I receive more time when running spline interpolation?}. For the purposes of OCR, \xxx{such complex and time-consuming functions are not desirable, as we are not yet working with any rotations and distortions and just need to resize the existing image.}\todo{nahradit necim jednoduchym jako ze `vyhody nejsou tak podstatny aby bylo potreba na ne pocitat tak slozity interpolace'} Therefore, the most popular decision for general cases is the use of Bicubic interpolation method~\cite{interpolationComp}.

\subsection{Contrast enhancement} \label{contrastEnhancemet}

\todo{je potreba nejdriv zadefinovat co je kontrast a proc ho potrebujes.}
\xxx{Another} important factor that results in low-quality images is low contrast\todo{word order!}, as it often results in possible blending of edges during recognition.

\xxx{An image histogram is used to improve this aspect.}\todo{no alternatives?} A histogram is an \xxx{accurate} representation of the distribution of data and, in case of image histograms, it represents the tonal distribution of an image --- x-axis stands for all available tonal levels, and y-axis represents the number of pixels for each tonal level. In colored (RGB) images, the histogram is displayed by the terms of three separate histograms, one for each color component.\todo{tenhle odstavec je extremne informal}

Contrast of the \xxx{current}\todo{current in what process? mozna to chces vynechat, nebo rict proste `a color component'} color component is represented as a distribution of pixel luminance values. By spreading out the most frequent intensity values in the histogram, as shown in~\cref{fig:preprocessContrastComparison}, the contrast of the image increases.
...
\caption{Histogram-based comparison of image contrast. Left: low-contrast image histogram, right: high-contrast image histogram. \xxx{Vypada to ze je mas obracene.}}
\label{fig:preprocessContrastComparison}
\end{figure}

\xxx{To expand the original digital values of histogram into a new distribution, different histogram spreading methods use functions of different forms. These can be either linear or non-linear, which leads to the categorization of histogram spreading methods into either}\todo{vynechat, Histogram stretching methods are categorized as...} \emph{linear stretching} methods or \emph{non-linear stretching} methods~\citep{linearNonStretch}.

\xxx{Both types of methods have their advantages and disadvantages.}\todo{obvious} Although linear methods are easier to implement and do not cause the objects in the original image to lose their brightness values, non-linear methods were shown to produce better results in general case~\citep{chandpa1comparative}. Despite of that, linear methods still deliver satisfactory results on \xxx{remotely sensed images with Gaussian or near-Gaussian histograms}\todo{co to znamena? aspon to remotely sensed je potreba bud odstranit nebo vysvetlit}.

The most popular choice for contrast enhancement for OCR is a non-linear technique called \emph{histogram equalization}~\citep{histogramEQ}. \xxx{Is is based on transforms that depend on the probability distribution function of the histogram.}\todo{moc genericky, bud ukazat matematiku nebo vynechat.} Although it produces unrealistic and over-enhanced effects in photographs, the over-processing actually suits the text data. The typical output of histogram equalization on textual data is displayed in~\cref{fig:preprocessHistogramEqualization}.

\xxx{Many other more complex and complicated techniques of contrast enhancement exist.}\todo{obvious, vsechno jde donekonecna komplikovat.. :D} \xxx{These were presented}\todo{mozna chces prepsat na aktivni, Suganya et al present ...} by~\citet{contrastOther} and contain techniques like CLAHE, Dynamic Stochastic Resonance and generic transforms, such as Fourier, Discrete Cosine, and Wavelet. For complex text recognition cases, CLAHE method can be used with satisfactory results.
...
To recognize characters in an image, an OCR software must distinguish the background pixels from the actual pixels that belong to the characters. \xxx{When it comes to}\todo{`when it comes to' znamena neco eskalativniho jako `kdyz dojde na lamani chleba' a naznacujes tim ze budou potreba extremni opatreni...coz binarizace obrazku uplne nevyzaduje} colored images, to determine the boundaries between these two groups \xxx{might be} a complex task due to the need of processing the whole RGB information of each pixel. This can be simplified by converting the image into grayscale mode, where we would need to process only one value, or even monochrome one mode, which would make the recognition algorithm trivial.\todo{tady je ta logika uplne obracene; tim ze barevnej obrazek predelas do greyscale muzes ztratit informaci napr. o cervenym textu na zelenym pozadi....}

\xxx{This is why it is beneficial for images to undergo the process of binarization, which converts images into their monochrome one form.}\todo{Tohle si odporuje s predchozim tvrzenim. Nechces rict neco jako: `Ackoliv by bylo obrazky jednoduchy hodit do greyscale anebo rovnou do black-white podle nejakyho thresholdu, tak to neni uplne vzdycky to co chcem protoze obrazky jsou slozitejsi (hlavne ty barevny).}

\xxx{Many binarization algorithms work under the assumption that the input image is already in grayscale mode, as it significantly simplifies their process. We therefore start this section by presenting individual techniques of grayscale conversion, and only then focus on the binarization methods.}\todo{redundant}

\subsubsection{Grayscale conversion} \label{grayscaleConversion}

Grayscale conversion is the process of computing a gray value for each \xxx{colored pixel from its attributes}. \xxx{There exist a few different approaches to this problem.}\todo{obvious} The simplest \xxx{one is}\todo{czenglish} the averaging approach, which assigns each pixel the average of its R, G and B value. However, human perception of colors is non-uniform --- green color is perceived more strongly than red, and red more strongly than blue\todo{to neni ten hlavni problem, problem je ze tim averagingem navic totalne zahodis rozdil mezi cervenejma a zelenejma pixlama, tohleto je vhodny do `additionally...'}. For this reason, a correction formula (often called \emph{luma}~\cite{grayscaleConv}) that weights each color component differently is used. Slight differences in the results of these techniques are demonstrated in~\cref{fig:preprocessGrayscale}.

\xxx{Another way}\todo{baf, zas neco dalsiho. Btw tyhle 2 odstavce muzes vklidu sprasknout do tvrzeni jako ze `slozitejsi metody reviewuje Cadik.'} to approach grayscale conversion is to represent the color based on its HSL values and convert it to its least saturated variant.

As described by~\citet{grayscaleCadik}, there exist many more algorithms that approach this problem. Most of them require more complicated computation\xxx{s}\todo{czenglish} and try to preserve attributes of the image that may be lost during the process, like contrast or luminance.
...
\xxx{Upon} converting the input image into grayscale, binarization \xxx{takes place}\todo{`takes place' je vhodny napr. pro korunovace a velky verejny slavnosti, tady asi chces rict ze kdyz uz je obrazek v greyscale tak ho muzes konecne prevyst na binarni background a foreground.}. Binarization methods\todo{all of them?} work on the basis of a \emph{threshold} --- a set\todo{mnozina?} value that a pixel is compared to\todo{just compared? tyhle 2 vety je asi vhodny spojit a podstatne zjednodusit}. If it is greater than the threshold, it is assumed to be white\todo{background?}, otherwise, it is colored black\todo{foreground?}. Various binarization methods differ in the way that this threshold is calculated and used.

The simplest approach to binarization is \emph{global thresholding}~\citep{globalThresh}\todo{word order!}. It works by choosing a threshold value and iterating over the whole image, comparing the value of each pixel to this threshold. If it is greater\todo{the value or the threshold? navic to rozdeleni na b/w uz mas v predchozim odstavci, netreba opakovat}, the pixel in the resulting image will be black, otherwise white. However, as the brightness and contrast of different images vary, it is generally impossible to choose a threshold suitable for all images.
...
Skewed images are probably the most common problem that many OCR engines struggle with. They usually appear when an image is scanned or photographed \xxx{crookedly}\todo{to zni jako crooked hillary. Myslis `from an angle'?}.

Negative effects of skewed images are most apparent in results of page segmentation (see \todo{sem hod ref na sekci se segmentaci}). Becase page segmentation techniques often rely on vertically and horizontally aligned characters and horizontal baselines of individual lines, skewed images \xxx{pass misaligned data}\todo{ty obrazky to tam nestrkaji, mozna chces jen rict ze ve skewed images je tyhle featury tezsi najit} to the OCR algorithm. \xxx{This causes more errors and ultimately leads to poorer results.}\todo{moc nespecificky, bud upresnit nebo vynechat}

\xxx{Therefore, to reconstruct the original alignment, rotation and skew of the image, various methods of deskewing are used (~\cref{fig:preprocessDeskewing}).} All of these methods work (for more accurate results) after binarization of the image and generally assume the skew angle to be no more than $20^{\circ}$. Although algorithms that work on greater skew angles also exist~\citep{skewAngleDetection}, they are not as widely used for the purposes of deskewing scanned documents, which are generally only slightly tweaked.
...
\end{itemize}
\xxx{However, worth noting}\todo{However znamena ze si to s necim odporuje, worth noting --- proc je nedas do toho seznamu a nemas tam normalne citace?} are also other methods like \emph{Fourier}~\citep{fourierTransform}, Wavelet Decomposition, Radon Transform, PCP, one-pass or multi-pass skew correction, morphology algorithms, etc.

\xxx{The mentioned methods mostly differ in the way they determine the skew of the image.}\todo{obvious} Hough transform applies feature extraction to the image and assigns the skew of the line with the most number of pixels to the document. Projection profile method rotates the image and finds the skew where its histogram along horizontal scan lines has the most peaks and valleys, while the cross correlation method uses this method and applies it to multiple vertical segments. Moreover, connected component clustering method is based on finding the coherency between connected components of the image and determining their common skew. We present the overview and comparison of these methods in~\cref{tab:preprocessSkewProsCons}.
...
Image noise is an usually unwanted random variation of brightness or color information in images, reminding\todo{reminding je od reminder, tadyto chces asi `similar to' nebo `reminiscent of'} film grain in digital cameras. It is often caused by the sensor of a scanner or a digital camera attempting to record small amounts of light.\todo{obracene: inevitable physical variance in amount of light recorded by a scanner or a digital camera?}
...
\emph{Non-linear filtering}, on the other hand, renders an output which is not a linear function of its input. Methods based on non-linear filtering used for denoising purposes include filters like \emph{median filter}, which computes the \xxx{results pixel's} value from a local median, or \emph{non-local means method}~\citep{nonLocalMeans}, which is based on taking a mean value of all pixels in the image weighted by their similarity to the target pixel. Both methods produce sharper and clearer images than linear filters at the expense of increased run time.
...
The goal of text detection is to recognize and classify individual textual elements of an image document and convert them into their digital form.\todo{ono digitalni uz jsou, mozna chces `interpret them as textual contents of the document' nebo tak cosi}

There exist two main approaches for text detection in OCR. \xxx{The first one is an approach based on heuristics. It includes techniques of page segmentation, line and element detection and character classification, which are based on different approximations and sometimes naive algorithms. The second one is an approach that is widely used and researched today --- and that is the use of neural networks.}\todo{implicitni seznam, prvni veta je matouci. Dal bych tam seznam: 1- heuristical statistics 2- neural-network-based}
...
\xxx{Bodky za jednotlivymi itemami? hej}
\begin{enumerate}
...
The task of page segmentation is to extract page sections that contain uniformly-formatted bodies of characters (e.g.~columns or paragraphs) that can be easily passed to later steps of text recognition. \xxx{This is, however, not always possible due to various errors that may occur during this process, which result in either merging or splitting the characters. Therefore, in OCR engines, page segmentation process usually means any kind of logical or topological division of the image into multiple smaller blocks --- which can be either characters, lines or even paragraphs.}\todo{redundant}
...
In this section, we will be concerned with a page segmentation technique proposed by~\citet{tesseractSegmentationTab}. This method is based on detecting \xxx{so-called}\todo{so-called is slightly derogative, also emph is redundant} \emph{tab-stops}.

Tab-stops are used in word processors to enable correct and \xxx{eye pleasing}\todo{kdo pouziva taby na alignment, zaslouzi shoret. (coz eventualne muze bejt eye-pleasing)} alignment of text, and are present in documents as margins, column edges, or indentations. All of them are placed at fixed x-positions, at which edges or centers of text lines are vertically aligned.
...
\begin{enumerate}
    ...
    \item Find the connected components of the document image \xxx{with the help of tab-stops}\todo{ale tady ty tabstopy jeste nemas? Nebo je magicky detekujes?}.
    \item Group the \xxx{found}\todo{discovered?} connected components into vertical lines (\cref{fig:segmentationTesseract2}).
