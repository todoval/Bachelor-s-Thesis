...
Binarization of an image is performed once the image is in grayscale mode. This can be achieved by methods 

\xxx{Upon} converting the input image into grayscale, binarization \xxx{takes place}\todo{`takes place' je vhodny napr. pro korunovace a velky verejny slavnosti, tady asi chces rict ze kdyz uz je obrazek v greyscale tak ho muzes konecne prevyst na binarni background a foreground.}. Most of the binarization methods work on the basis of a \emph{threshold} --- a value that determines whether the pixel should be black or white in the resulting binarized image.


pixel is compared to\todo{just compared? tyhle 2 vety je asi vhodny spojit a podstatne zjednodusit}. If it is greater than the threshold, it is assumed to be white\todo{background?}, otherwise, it is colored black\todo{foreground?}. Various binarization methods differ in the way that this threshold is calculated and used.
