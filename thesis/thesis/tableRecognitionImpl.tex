...
\xxx{Na chvilu odbocime od temy? ako to poviem? alebo kam to dat?}
...
\begin{enumerate}
    ...
    Already discussed in \xxx{taggnut sekciu}, page layout analysis is the first and one of the most crucial steps to an OCR algorithm. Line detection relies on this step to provide meaningful text regions.
    
    \item \emph{Line Finding}
     
     Upon obtaining text regions, heuristics like height filter and median filter are used to estimate the height of characters and remove noise, punctuation and other \xxx{problems??}. The filtered so-called \emph{blobs} are then merged into lines by their y-coordinate and position.
