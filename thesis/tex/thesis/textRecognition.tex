...
\citet{preprocessAll} describe the following major difficulties that complicate the text recognition process: \todo{nasledujici seznam se nesmi pouzivat jako neviditelny vetny clen, vety je potreba upravit nejak rozumne (idealne takhle)}

\todo{tadyto je optimalni tvar vyjmenovavaciho seznamu. "nadpisy" jsou navigace, tj. opet nejsou soucast vety.}
\begin{description}
...
\item[Handwritten text] \xxx{isn't this subsumed by the previous item?} The software has no notion about whether the document was handwritten or not. It sees the characters as merely a more complicated font. On contrary to the font, the characters that are supposed to be the same do not always have to look that way. This is why the OCR software has to count on minor character mistakes. These depend on different factors - line height, length, character size... Furthermore, handwriting can sometimes be unrecognizable even by a human eye. This case often prompts the software to fail.
...
\item[Skew problems] Images that are skewed also \xxx{fall (czenglish) under} the part of preprocessing. As the result of a scan or even a photography is almost never an image with a perfectly aligned text as it was on the paper, this needs to fixed, so the actual algorithm can count on correctly aligned images.
\item[Colors] \xxx{The general rule is -}\todo{na tohle bacha: jednopomlcka je na spojovany-slova, dvojpomlcka je na rozsahy od--do napr. 5--10, trojpomlcka je presne ta ktera slouzi na pomlku ve vetach --- takhle.} the better the contrast of the image, the better the OCR results. Colored images generally have a lower contrast than when they are in black and white\todo{can't color information be converted to something meaningful?}. Before passing to OCR, they therefore undergo binarization.
...
These and multiple other factors are also the reason why the majority of OCR software works with assumptions \todo{uvozovky se pisou `takhle' nebo ``takhle''} about documents --- \todo{tady jsem hodil spravnou pomlcku} that the document has only one column, is not handwritten, that its lines are horizontally aligned, \xxx{et cetera}\todo{latinsky veci se pisou v italics, ale spis se na to vykasli a pis normalne zkratky e.g., i.e., nebo etc.; tak to je vetsinou nejlepsi. Uplne nejlepsi je se obejit bez latiny, kdyz uz ta prace je anglicky.}. Also, this is why no OCR software can work perfectly and will always keep encountering documents that are unrecoverable. 

However, there are many ways in which we can improve the results of the OCR. \todo{which ones? je fajn rict neco jako `ty moznosti jsou tyhle a tyhle, a budeme si o nich ted chvili povidat}
...
Many popular OCR engines (e.g. Tesseract \citep{TesseractQual}, ABBYY FineReader \citep{ABBYYdpi}) encourage their users to use \xxx{a} 300dpi images. Their reason for that is pretty simple - it is the point where \xxx{you}\todo{who?} gain the most accuracy without sacrificing speed and file size. After running a few tests for the same image with different resolution on OCR engines \citep{preprocessAll}, the improvement gap between 200dpi scan and 300dpi scan is at least 2 times the improvement gap of any other resolutions. Also, when comparing other images above 300dpi with a 300dpi image, the improvement gap is nearly absent. It is obviously still there, but in almost all cases, the higher resolution is not worth the time and space. 
...
Achieving \todo{Achievement unlocked! you probably wanted to say `\underline{Obtaining} a 300dpi scan using currently available hardware...'?} a 300dpi by a scan is simple enough. If the user has already provided us with an image of a different resolution, our next steps depend on what the resolution is.
\begin{description}
\item[Lower-resolution images] If we \xxx{were to} \todo{werewolf-style} just scale the image, the result would be pixelated, blurred and not sharp enough. The most affected parts would be the diagonal edges of elements. They would appear jagged (\emph{aliased}), which gives the image overall a poor quality. To minimize this unwanted effect, a technique called \emph{interpolation} is used.
...
Non-adaptive methods do not distinct different pixels. Their complexity depends on the number of adjacent pixels during interpolation, which is also the criterion by which the existing methods are divided~\cite{interpolation}.\todo{note: `divide' implicitly means `reduce' in english, you want `categorize' or `classify'.}

\begin{itemize}
\item\textbf {Nearest Neighbor Interpolation: } This \todo{Which? Text in bold is only a navigation, not information. Corrected version provided below.} algorithm considers only one pixel - the closest one to the interpolated point.
...
\item\textbf {Bilinear Interpolation } considers the closest 2\xxx{x}\todo{Mame celej latex lepsich znaku nez iks. Treba $2\cdot2$ nebo $2\times2$ nebo 2\texttimes2}2 neighborhood of known pixel values surrounding the unknown pixel. It then assigns the weighted average of these 4 pixels to the final interpolated value.
...
There also exist higher order interpolations, such as \emph{Spline}, \emph{Sinc}, \emph{Lanczos}, \emph{Discrete Wavelet Transforms}~\cite{interpolation}\xxx{...} These consider more surrounding pixels and calculate the resulting value through more complex functions, like transforms or sampling functions. The results are more accurate than those of simple calculations, but \todo{at the cost of?} the process takes up significantly more time. \xxx{During OCR}\todo{for the purposes of?}, such complex and time-consuming functions are not necessary\todo{if they weren't necessary, why would anyone spend time inventing those? you probably mean `desirable' or `tolerable'}, as we are not yet working with any rotations and distortions and just need to resize the existing image.
...
\textbf{Grayscale conversion} is the process of computing and assigning the grey value for each colored pixel from its attributes. Over the years, different approaches have been created. That is why, when using image editing software such as Adobe Photoshop or GIMP\footnote{\url{http://www.gimp.malovani/}}\todo{at least a link to the website is required, as shown}, there are many options available for grayscale conversion, each of them producing slightly different results.\todo{this paragraph is probably kindof self-explanatory and redundant}

The most simple approach is to find the average of the R, G and B values. However, this does not really preserve the brightness of the image. \xxx{Therefore}\todo{for that reason, more advanced corrections based on luma...?}, corrections like \emph{luma}~\cite{grayscaleConv} are used. This algorithm is based on the fact that humans perceive different colors \todo{did you mean the intensities of those? perceiving just `different colors differently' is kindof self-evident.} differently (green more strongly than red, and red more strongly than blue), therefore, each color is weighted dependant on how it is perceived by the human eye. Another way to approach grayscale converstion is to represents the color based on its HSL values and convert it to its least saturated variant.
...
To \xxx{achieve} \todo{Achievement is something that plays the `achievement unlocked!' sound. This is just `obtain'.} an image with \xxx{a} higher contrast, we therefore need to stretch its histogram. In the following paragraph, we will briefly explain the basis of a few of the most popular methods. However, many more complex and complicated techniques already exist, including
...
\item\textbf{Histogram equalization} is one of the most popular methods of non-linear stretching\todo{this would mean that it should go to the previous list item. You probably want to say `is the most popular method based off the non-linear stretching?}. It is based on transforms \xxx{dependant} \todo{that depend} on the probability distribution function of the histogram~\citep{histogramEQ}. Although it produces unrealistic effects in photographs, it is widely used for scientific images \todo{....and text?}.
...
\section{Basic techniques \xxx{(...for what?)}}
...
\subsection{Deep \xxx{L}earning}
-- TO-DO - research deep learning for other purposes, maybe for the whole OCR

\xxx{No one really likes deep learning for being too slow and fragile. I would either omit that completely, or place the whole thing into some kind of `alternate reality' chapter.}

\section{Available \xxx{I}mplementations}
\todo{Please remove news-style caps from all headers. It's good for tabloid news but quite ugly in any serious literature. Also, this would probably better be named as `Available OCR software'}
